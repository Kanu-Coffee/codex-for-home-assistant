...
현재 공개 사전 릴리스는 [`0.3.1`](https://github.com/Kanu-Coffee/codex-for-home-assistant/releases/tag/0.3.1)입니다. 정확한 공개 이미지의 자동 회귀는 PASS했습니다. 제공된 실제 HAOS `0.3.0` 읽기 전용 감사에서는 catalog refresh가 모두 `ha_unavailable`로 실패했으며, legal null config와 진단 경로를 보완한 `0.3.1` 공개 image의 실제 HAOS catalog/restart/candidate/change/privacy 재시험은 아직 **NOT RUN**입니다. `stage: experimental`, amd64 전용이며 Supervisor `admin`, Docker API, App `full_access`, host network는 사용하지 않습니다.
...
공개 `0.3.1` amd64 image의 인증 없는 generic/per-arch resolve·pull과 자동 회귀는 **PASS**였습니다. generic/per-arch OCI index digest는 `sha256:6e726ad1ce95714fb4d4f29dcbb51f762cdd3e7b3eeb9c1be7edcbb6ff1c0126`, runtime manifest digest는 `sha256:175c1a2b4d45897f713ab37f71c96b96c9b182e2f74340843c18aa34e7d84954`이며 mutable `latest` tag는 게시하지 않았습니다. 정확한 공개 이미지에서 memory와 실제 installed `ws`, browser/gateway/Core WebSocket/ttyd/SSH, managed-auth, user-file update와 public `0.3.0` → `0.3.1` update smoke를 확인했고 동일 merge SHA의 [main CI 29391446446](https://github.com/Kanu-Coffee/codex-for-home-assistant/actions/runs/29391446446)과 [Builder 29391579529](https://github.com/Kanu-Coffee/codex-for-home-assistant/actions/runs/29391579529)이 PASS했습니다. 실제 HAOS `0.3.0` 읽기 전용 감사의 memory catalog는 **FAIL**했고, 공개 `0.3.1`의 catalog/restart/candidate/change/privacy 실기 재시험은 **NOT RUN**입니다. 기존 `0.2.3` browser/AppArmor 사용자 확인 PASS와 이 memory 실패를 혼합하지 않습니다.
