...
현재 공개 사전 릴리스는 [`0.2.3`](https://github.com/Kanu-Coffee/codex-for-home-assistant/releases/tag/0.2.3)이며, 이 소스는 HAOS 실기 완료 기록을 포함한 `0.2.4` 릴리스 후보입니다. `stage: experimental`, amd64 전용이며 공개 이미지 자동 검증에 더해 Home Assistant 구성 UI/Supervisor 일반 업데이트와 AppArmor 활성 상태의 인증된 `8099` dashboard desktop/mobile·console·network/resource·WebSocket 경로를 사용자가 **PASS**로 확인했습니다. Supervisor `admin`, Docker API, App `full_access`, host network는 사용하지 않습니다.
...
Supervisor는 public generic manifest `ghcr.io/kanu-coffee/codex-for-home-assistant:0.2.3`을 내려받습니다. 이 tag는 공식 Home Assistant builder action으로 게시됐으므로 App Store 설치 중 소스 컴파일을 요구하지 않습니다. 실제 HAOS 기능 결과는 `progress.md`의 M2 항목별 증거를 기준으로 합니다.
...
공개 `0.2.3` amd64 image의 인증 없는 generic/per-arch pull과 전체 Docker smoke는 **PASS**입니다. OCI index digest는 `sha256:a53c7b2006301826a52bc9d9dc3c3ec8fd5e99d73b59a028b16c78bf7628d2a1`이며 mutable `latest` tag는 게시하지 않았습니다. 공개 이미지에서 managed-auth lifecycle, user-file backup/recovery, desktop `1440x900`·mobile `390x844` PNG, console/network, direct Core REST/WebSocket, token redaction과 public `0.2.2` → `0.2.3`의 기본 preserve→명시적 refresh→same-version restart update smoke를 확인했습니다. 공식 Builder run은 [29334867268](https://github.com/Kanu-Coffee/codex-for-home-assistant/actions/runs/29334867268)입니다. Playwright upstream은 Alpine/musl을 공식 browser 실행 플랫폼으로 지원하지 않으며 이 구현은 Playwright 번들 대신 Alpine Chromium을 사용합니다. 실제 Home Assistant 구성 UI/Supervisor 일반 업데이트와 AppArmor 활성 상태의 인증된 `8099` dashboard desktop/mobile, console, network/resource, WebSocket 경로는 사용자 확인 **PASS**입니다. 상세 실행 로그와 HAOS 버전은 제공되지 않아 추정하거나 저장소 증거로 기록하지 않습니다.
