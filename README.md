...
현재 공개 사전 릴리스는 [`0.3.0`](https://github.com/Kanu-Coffee/codex-for-home-assistant/releases/tag/0.3.0)입니다. `0.3.0`은 검증 기반 HA 구조·사용자 의미 메모리를 추가하며 `stage: experimental`, amd64 전용입니다. 정확한 공개 이미지의 메모리·browser/gateway/Core WebSocket/ttyd/SSH·managed-auth·user-file·`0.2.4 → 0.3.0` 업데이트 자동 검증은 **PASS**지만, 새 메모리 기능의 실제 HAOS Core registry/restart/update E2E는 **NOT RUN**입니다. Supervisor `admin`, Docker API, App `full_access`, host network는 사용하지 않습니다.
...
`0.3.0`으로 App version이 바뀌므로, `refresh_agents` 또는 `refresh_all`을 계속 선택한 설치는 선택 target을 `0.3.0`에서 한 번 다시 갱신합니다. 재적용을 원하지 않으면 **업데이트 전** App **구성**에서 `preserve`로 저장하세요.
...
공개 `0.3.0` amd64 image의 인증 없는 generic/per-arch resolve·pull과 version/arch/source label 확인은 **PASS**입니다. generic/per-arch OCI index digest는 `sha256:5ddb290534d45d46b34d2ef478ede83110c3b8e3a56f601ce2c6b45d5814341f`, runtime manifest digest는 `sha256:1c1e6a43cf24508e1c39c62bb2f0ab57c635208d8bd47537d4f9d09f0487f360`이며 mutable `latest` tag는 게시하지 않았습니다. 정확한 공개 이미지에서 memory, browser/gateway/Core WebSocket/ttyd/SSH, managed-auth, user-file update와 public `0.2.4` → `0.3.0` update smoke를 확인했고, 동일 merge SHA의 native Linux [main CI 29384344027](https://github.com/Kanu-Coffee/codex-for-home-assistant/actions/runs/29384344027)도 PASS했습니다. 공식 Builder run은 [29384484760](https://github.com/Kanu-Coffee/codex-for-home-assistant/actions/runs/29384484760)입니다. Playwright upstream은 Alpine/musl을 공식 browser 실행 플랫폼으로 지원하지 않으며 이 구현은 Playwright 번들 대신 Alpine Chromium을 사용합니다. 실제 Home Assistant 구성 UI/Supervisor 일반 업데이트와 AppArmor 활성 상태의 인증된 `8099` dashboard 경로는 public `0.2.3`에서 사용자 확인 **PASS**지만 새 메모리의 실제 HAOS Core registry/restart/update E2E는 **NOT RUN**입니다.
