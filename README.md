...
현재 공개 사전 릴리스는 [`0.2.3`](https://github.com/Kanu-Coffee/codex-for-home-assistant/releases/tag/0.2.3)입니다. `stage: experimental`, amd64 전용이며 관리형 browser 인증, 기본 ON 자동화와 선택형 user-file refresh는 공개 이미지의 자동 검증과 실제 Home Assistant UI/HAOS AppArmor dashboard E2E를 분리해 기록합니다. AppArmor는 활성화되어 있고 Supervisor `admin`, Docker API, App `full_access`, host network는 사용하지 않습니다.
