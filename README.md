...
현재 공개 사전 릴리스는 [`0.3.2`](https://github.com/Kanu-Coffee/codex-for-home-assistant/releases/tag/0.3.2)이며 정확한 공개 이미지 자동 회귀는 PASS했습니다. 이전 public `0.3.1`의 실제 HAOS/Core `2026.7.2` 실기에서는 automation-related 30건 중 2건의 `unknown_error` 때문에 catalog bootstrap이 **FAIL**했고 Core restart는 PARTIAL이었습니다. `0.3.2`는 해당 실패를 격리해 공개 image 회귀까지 통과했지만 실제 HAOS catalog/LKG/restart/CLI·MCP/candidate/change/App restart·update/privacy 재시험은 아직 **NOT RUN**입니다. `stage: experimental`, amd64 전용이며 Supervisor `admin`, Docker API, App `full_access`, host network는 사용하지 않습니다.
...
Supervisor는 public generic manifest `ghcr.io/kanu-coffee/codex-for-home-assistant:0.3.2`를 내려받습니다. 이 tag는 공식 Home Assistant builder action으로 게시됐으므로 App Store 설치 중 소스 컴파일을 요구하지 않습니다. 실제 HAOS 기능 결과는 `progress.md`의 M2 항목별 증거를 기준으로 합니다.
...
Public `0.3.2`로 업데이트할 때 `refresh_agents` 또는 `refresh_all`을 계속 선택한 설치는 선택 target을 `0.3.2`에서 한 번 다시 갱신합니다. 재적용을 원하지 않으면 **업데이트 전** App **구성**에서 `preserve`로 저장하세요.
...
공개 `0.3.2` amd64 image의 인증 없는 generic/per-arch resolve·pull과 자동 회귀는 **PASS**였습니다. generic/per-arch OCI index digest는 `sha256:d14bb71190be15fa6a45a19a2e981cb173c741e37ab0344c9bf941abbaab2c6b`, runtime manifest digest는 `sha256:3af26175df8c35a83e19634a0f7278f3ca3f70b08cb035e9953f2a519a9f353a`이며 mutable `latest` tag는 게시하지 않았습니다. 정확한 공개 이미지에서 memory와 실제 installed `ws`, browser/gateway/Core WebSocket/ttyd/SSH, managed-auth, user-file update와 public `0.3.1` → `0.3.2` update smoke를 확인했고 동일 merge SHA의 [main CI 29404524753](https://github.com/Kanu-Coffee/codex-for-home-assistant/actions/runs/29404524753)과 [Builder 29404702270](https://github.com/Kanu-Coffee/codex-for-home-assistant/actions/runs/29404702270)이 PASS했습니다. 별도 실제 HAOS `0.3.1` 실기에서는 Core 연결과 privacy가 PASS했지만 catalog/LKG/실제 CLI·MCP 조회가 related `unknown_error` 때문에 **FAIL**, Core restart는 **PARTIAL**, null-config는 **NOT OBSERVED**, candidate/change/App restart/update는 **NOT RUN**입니다. Public `0.3.2`의 실제 HAOS 재시험은 아직 **NOT RUN**이며 기존 `0.2.3` browser/AppArmor 사용자 확인 PASS와 혼합하지 않습니다.
