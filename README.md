...
현재 공개 사전 릴리스는 [`0.2.2`](https://github.com/Kanu-Coffee/codex-for-home-assistant/releases/tag/0.2.2)이고 이 소스의 다음 후보는 `0.2.3`입니다. `stage: experimental`, amd64 전용이며 관리형 browser 인증, 기본 ON 자동화와 선택형 user-file refresh는 local container 검증과 실제 Home Assistant UI/HAOS AppArmor dashboard E2E를 분리해 기록합니다. AppArmor는 활성화되어 있고 Supervisor `admin`, Docker API, App `full_access`, host network는 사용하지 않습니다.
...
Local `0.2.3` 후보의 기본 ON browser 인증과 8099 route는 managed-auth smoke, full Docker smoke와 public `0.2.2` replacement smoke를 통과했습니다. 선택형 user-file refresh의 최종 자동 검증 결과는 `progress.md`를 기준으로 하며 실제 Home Assistant 구성 UI/Supervisor update와 사용자 HAOS/AppArmor dashboard는 별도 **NOT RUN**입니다. browser fixture에서는 option 누락/default ON 자동 생성, 재시작 동일 identity 재사용, OFF/ON 보존·재활성화, ON 삭제 거부·OFF 삭제, 수동 override 억제와 model-visible `8099` 지침을 확인했습니다. 내부 gateway는 desktop `1440x900`·mobile `390x844` 실제 PNG, console/network, direct Core REST/WebSocket까지 통과했습니다. 이 결과는 실제 사용자 HAOS/AppArmor dashboard 실기 PASS를 뜻하지 않습니다.
