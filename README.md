...
현재 공개 사전 릴리스는 [`0.3.0`](https://github.com/Kanu-Coffee/codex-for-home-assistant/releases/tag/0.3.0)이고 다음 patch 후보는 `0.3.1`입니다. `0.3.0` 정확한 공개 이미지의 자동 회귀는 PASS였지만 제공된 실제 HAOS 읽기 전용 감사에서는 catalog refresh가 모두 `ha_unavailable`로 실패했습니다. `0.3.1`은 unavailable automation의 legal null config, 고정 `ws` transport와 안전한 단계별 진단을 보완하며 공개 image의 실제 HAOS 재시험 전까지 live 완료로 간주하지 않습니다. `stage: experimental`, amd64 전용이며 Supervisor `admin`, Docker API, App `full_access`, host network는 사용하지 않습니다.
...
Supervisor는 public generic manifest `ghcr.io/kanu-coffee/codex-for-home-assistant:0.3.0`을 내려받습니다. 이 tag는 공식 Home Assistant builder action으로 게시됐으므로 App Store 설치 중 소스 컴파일을 요구하지 않습니다. 실제 HAOS 기능 결과는 `progress.md`의 M2 항목별 증거를 기준으로 합니다.
...
`0.3.1`로 App version이 바뀌면 `refresh_agents` 또는 `refresh_all`을 계속 선택한 설치는 선택 target을 `0.3.1`에서 한 번 다시 갱신합니다. 재적용을 원하지 않으면 **업데이트 전** App **구성**에서 `preserve`로 저장하세요.
...
`0.3.1`에서는 unavailable automation이 Core에서 `config: null`을 반환해도 entity와 related graph를 유지합니다. 실제 실패는 `ha_token_unavailable`, `ha_auth_rejected`, DNS/transport/timeout/protocol 또는 고정 command/snapshot code로 status에 구분되며 daemon log에는 원격 응답이나 token 대신 allowlist code만 남습니다. memory endpoint는 고정 Supervisor proxy이고 `HA_WS_URL` 환경변수로 변경할 수 없습니다.
...
공개 `0.3.0` amd64 image의 인증 없는 generic/per-arch resolve·pull과 자동 회귀는 **PASS**였습니다. generic/per-arch OCI index digest는 `sha256:5ddb290534d45d46b34d2ef478ede83110c3b8e3a56f601ce2c6b45d5814341f`, runtime manifest digest는 `sha256:1c1e6a43cf24508e1c39c62bb2f0ab57c635208d8bd47537d4f9d09f0487f360`이며 mutable `latest` tag는 게시하지 않았습니다. 정확한 공개 이미지에서 memory, browser/gateway/Core WebSocket/ttyd/SSH, managed-auth, user-file update와 public `0.2.4` → `0.3.0` update smoke를 확인했고 동일 merge SHA의 [main CI 29384344027](https://github.com/Kanu-Coffee/codex-for-home-assistant/actions/runs/29384344027)과 [Builder 29384484760](https://github.com/Kanu-Coffee/codex-for-home-assistant/actions/runs/29384484760)이 PASS했습니다. 이후 실제 HAOS 0.3.0 읽기 전용 감사의 memory catalog는 **FAIL**했고, 0.3.1 공개 image의 catalog/restart/update 실기 재시험은 **NOT RUN**입니다. 기존 `0.2.3` browser/AppArmor 사용자 확인 PASS와 이 memory 실패를 혼합하지 않습니다.
