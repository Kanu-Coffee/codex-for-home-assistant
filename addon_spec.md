...
로컬 개발 단계에서는 local build를 허용한다. 첫 non-dev 배포 시 GHCR multi-arch 또는 amd64 image를 GitHub Actions로 미리 빌드하고 `config.yaml`의 `image` 필드를 사용한다. 이미 배포된 개발판보다 낮은 version으로 되돌리지 않는다. Home Assistant `stage`는 M3 평가 전까지 `experimental`을 유지한다.
