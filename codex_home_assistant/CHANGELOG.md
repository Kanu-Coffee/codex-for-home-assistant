...
- Replace the ineffective capability experiment with AppArmor stable `4.0.3`'s official bubblewrap user-namespace profile, fetched only on CI hosts from an immutable commit and verified by SHA-256 before loading. Keep the test container unprivileged. Make the reusable build inherit the caller's token: PR dry-runs remain read-only, while only tag publication receives package, OIDC, attestation, and artifact-metadata write scopes. Local unit/contract, workflow lint, pinned profile parse/checksum, and managed sandbox smoke pass; updated remote PR validation remains pending.
- An exact `0.7.0-dev.1` GHCR image and published multi-architecture manifest, native/release scans, and real 64-bit Raspberry Pi HAOS installation/AppArmor/runtime acceptance remain **NOT RUN**. PR #35 is open; no merge, `0.7.0-dev.1` tag, image or attestation publication, or release has been performed.
