...
> 현재 공개 사전 릴리스 `0.2.4`는 amd64 전용이며 Home Assistant `stage: experimental`을 유지합니다. `0.2.4`는 런타임 기능 변경 없이 public `0.2.3`의 HAOS 실기 완료 기록을 반영한 validation/evidence patch입니다. 관리형 최소권한 browser 인증, 기본 ON 자동화와 선택형 user-file refresh는 public image container smoke와 사용자가 확인한 Home Assistant UI/HAOS AppArmor 실기 결과를 구분해 기록합니다. `/config` 전체를 쓰고 Home Assistant Core API와 Supervisor `manager` API를 호출할 수 있는 강한 관리자 도구이므로 인터넷에 SSH 포트를 직접 공개하지 마세요.
...
`config.yaml`은 public generic manifest `ghcr.io/kanu-coffee/codex-for-home-assistant`를 사용합니다. Supervisor는 App version과 같은 공개 `0.2.4` tag의 미리 빌드된 amd64 이미지를 내려받으므로 설치 장치에서 소스 빌드를 하지 않습니다.
...
`0.2.4`는 런타임 기능 변경 없는 validation/evidence patch이지만 App version이 바뀌므로 target별 1회 규칙은 다시 적용됩니다. 현재 `refresh_agents` 또는 `refresh_all`을 계속 선택했다면 선택 target이 `0.2.4`에서 한 번 다시 갱신됩니다. 이 재적용을 원하지 않으면 **업데이트 전** App **구성**에서 `codex_user_files_update_mode: preserve`로 저장하세요.
...
Public `0.2.4`의 관리형 user/token 생성·재사용·회전·제거, 강제 종료 임시파일 회수, 응답 유실·지속 cleanup 실패 복구, App replacement와 동시 실행 차단, Core/provider 장애 및 정책 변조 fail-closed를 fixture에서 확인했습니다. 같은 public image에서 option 누락/default ON 자동 생성, restart 재사용, OFF 상태의 runtime 차단·identity 보존, ON 재활성화, 수동 override 억제와 model-visible 8099 routing도 검증했습니다. 정확한 공개 이미지의 managed-auth, user-file update와 public `0.2.3` → `0.2.4` update/MCP smoke가 PASS했고, 동일 merge SHA의 native Linux main CI에서 `http://127.0.0.1:8099` fixture의 desktop/mobile PNG, console/network, direct Core REST/WebSocket와 ttyd/SSH를 포함한 전체 컨테이너 회귀가 PASS했습니다. 공식 Builder run [`29339603324`](https://github.com/Kanu-Coffee/codex-for-home-assistant/actions/runs/29339603324)이 게시한 generic/per-arch index digest는 `sha256:3f9d70943ad68f2b7177826546227fa00c68068b57bdf7b1686edf1c3de90f26`, runtime manifest digest는 `sha256:e435844c06d7813ad527e3897dd647cc313a03d7c7c833301953b253a57658a8`이며 익명 pull과 version/arch/source label을 확인했습니다.
...
이 MVP에는 전용 `ha-ws` helper나 범용 WebSocket CLI가 포함되지 않았습니다. 필요한 경우 token redaction을 지키는 코드/클라이언트를 사용하세요. Ingress WebSocket transport와 인증된 `8099` dashboard의 Core WebSocket 경로는 HAOS에서 사용자 확인 PASS이지만, 범용 Core WebSocket CLI/helper를 실기 검증했다는 뜻은 아닙니다.
...
| 실제 HAOS 일반 App 업데이트 후 MCP 노출 | **PASS — Home Assistant 구성 UI·Supervisor 일반 update 사용자 확인** |
| 실제 HAOS `8099` dashboard 인증·desktop/mobile·console·network/resource·WebSocket E2E | **PASS — AppArmor 활성 상태 사용자 확인** |

위 상태는 이 문서 작성 시점의 사실이며 최종 명령·결과는 저장소 `progress.md`가 기준입니다. 기존 `0.1.3` 범위의 amd64 runtime M1/M2와 public `0.2.0` browser renderer, `0.2.1` 최소권한 경로, `0.2.2` 관리형 인증, public `0.2.3` 자동화·user-file refresh 및 public `0.2.4` validation/evidence 회귀는 PASS입니다. 기존 설치의 Home Assistant 구성 UI/Supervisor 일반 업데이트와 AppArmor 활성 상태의 인증된 `8099` dashboard desktop/mobile·console·network/resource·WebSocket 경로는 public `0.2.3`에서 사용자 확인 PASS입니다. 현재 공개 사전 릴리스는 `0.2.4`입니다. 새 실기 확인의 상세 로그와 HAOS 버전은 제공되지 않아 기록하지 않았습니다. Supervisor/Core/App lifecycle 실동작, 실제 물리 장치 제어와 다른 별도 미실행 항목은 운영 중단·오작동 위험 때문에 자동 완료 시험에서 제외합니다. Home Assistant `stage`는 M3 평가 전까지 `experimental`을 유지합니다.
