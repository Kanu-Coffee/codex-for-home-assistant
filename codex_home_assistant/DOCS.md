...
> 현재 공개 사전 릴리스 `0.2.1`과 이 소스의 `0.2.2` 후보는 amd64 전용이며 Home Assistant `stage: experimental`을 유지합니다. `0.2.2`의 자동 최소권한 browser 인증은 local container smoke와 실제 HAOS 실기 결과를 분리해 기록합니다. `/config` 전체를 쓰고 Home Assistant Core API와 Supervisor `manager` API를 호출할 수 있는 강한 관리자 도구이므로 인터넷에 SSH 포트를 직접 공개하지 마세요.
...
`config.yaml`은 public generic manifest `ghcr.io/kanu-coffee/codex-for-home-assistant`를 사용합니다. Supervisor는 App version과 같은 공개 `0.2.1` tag의 미리 빌드된 amd64 이미지를 내려받으므로 설치 장치에서 소스 빌드를 하지 않습니다.
...
### 권장: 자동 read-only browser 사용자 설정

App Web terminal에서 인수 없이 다음 명령을 한 번 실행합니다.
...
설치·업데이트·재시작만으로 Home Assistant user/token을 만들거나 provider·proxy 설정을 바꾸지는 않습니다. `ha-browser-auth-setup`은 관리자가 Web terminal에서 명시적으로 실행한 경우에만 아래 transaction을 시작합니다. 최초 설정 이후의 재시작·업데이트는 이미 준비된 관리 상태를 검증해 재사용할 뿐입니다.

`ha-browser-auth-setup`은 Home Assistant의 지원되는 admin/user WebSocket과 `/auth/login_flow`, `/auth/token`, `/auth/revoke`만 사용해 다음 작업을 자동으로 수행합니다.
...
사용자가 Home Assistant profile에서 token을 발급하거나 복사·붙여넣기할 필요가 없습니다. App 재시작·업데이트와 각 Playwright MCP 시작 때 같은 identity를 재검증해 재사용합니다. 중간에 Core/DNS/TLS가 불안정하거나 user policy·credential이 바뀌면 영구 복구 자료를 성급히 삭제하지 않고 `/run` token만 제거해 dashboard 자동 로그인을 차단합니다. 동시에 두 setup/remove가 실행되지 않도록 kernel lock을 사용합니다.

`status: ready`, `source: managed`, `group_ids: ["system-read-only"]`, `local_only: true`, `is_admin: false`가 모두 보여야 합니다. 관리형 identity를 더 이상 쓰지 않을 때는 다음 명령으로 App이 만든 user와 token을 정책 확인 후 제거합니다.

```bash
ha-browser-auth-remove
```

사용자가 관리형 user의 이름, group, local-only 상태나 credential을 수동 변경한 경우 자동 삭제·수리를 거부할 수 있습니다. 먼저 변경 내용을 검토하고 원래 exact policy로 복구한 뒤 제거를 다시 실행하세요. App 완전 삭제 전에 관리형 identity가 필요 없다면 `ha-browser-auth-remove`를 먼저 실행하는 것이 좋습니다.

### 수동 token override/fallback

기존 optional `home_assistant_browser_token`도 계속 지원합니다. 이 App option을 설정하면 관리형 token보다 우선하며, 다음 조건을 모두 만족할 때만 자동 로그인이 켜집니다.
...
Local `0.2.2` 최종 후보 image는 관리형 user/token 생성·재사용·회전·제거, 강제 종료 임시파일 회수, 응답 유실·지속 cleanup 실패 복구, App replacement와 동시 실행 차단, Core/provider 장애 및 정책 변조 fail-closed를 fixture에서 확인했습니다. `http://127.0.0.1:8099` fixture에서 desktop/mobile PNG를 실제 저장하고 console/network와 direct Core REST/WebSocket도 확인했습니다. 설치·업데이트·재시작 자체는 user/token을 만들지 않는 계약이며, 공개 `0.2.1`에서 최종 candidate로 교체하는 replacement smoke도 같은 `/data`·`/config` 보존 조건으로 통과했습니다.
...
4. Home Assistant dashboard라면 `ha-browser-network-info`와 `ha-browser-auth-status`를 확인한 다음 App shell에서 `http://127.0.0.1:8099`가 응답하는지 봅니다. `unconfigured`이면 `0.2.2` 후보의 Web terminal에서 `ha-browser-auth-setup`을 명시적으로 한 번 실행하거나 수동 override를 설정합니다. provider 부재, Core TLS 또는 정책 오류는 우회하지 말고 원인을 먼저 고치세요. browser token이나 `SUPERVISOR_TOKEN` 값을 직접 출력하거나 URL/query에 붙이지 마세요.
...
Browser token 노출이 의심되면 실행 중인 Codex/browser 세션을 종료합니다. `ha-browser-auth-status`의 `source`가 `managed`이면 `ha-browser-auth-remove`로 App이 관리하는 user와 token을 함께 제거하고, 제거가 정책 불일치로 거부되면 App을 중지한 채 Home Assistant 관리자 UI에서 해당 identity와 token을 직접 검토·폐기합니다. `source`가 수동 option이면 전용 사용자의 profile에서 해당 long-lived token을 삭제하고 `home_assistant_browser_token` option도 비운 뒤 재시작합니다. `8099`가 login page로 fail closed되는지 확인한 뒤 관리형 방식은 `ha-browser-auth-setup`, 수동 방식은 새 token으로 다시 설정합니다. read-only token과 `/data/browser-auth` backup도 모든 entity state에 접근할 수 있는 민감자료로 취급하세요.
...
| `0.2.2` 관리형 browser 인증 후보 | **PASS (최종 local fixture) — user/token 생성·재사용·회전·제거, crash/응답 유실/cleanup 실패 복구, 동시 실행 차단, Core/provider 장애·정책 변조 fail-closed** |
...
| Public `0.2.1` → local `0.2.2` container update | **PASS — 동일 `/data`·`/config`, Codex auth/config/지침/SSH identity와 App option 보존, 새 managed-auth helper와 MCP 실행** |
| 실제 HAOS 일반 App 업데이트 후 MCP 노출 | **NOT RUN — Supervisor/App Store 경로 미검증** |
| 실제 HAOS `8099` dashboard 인증·resource·WebSocket E2E | **NOT RUN** |

위 상태는 이 문서 작성 시점의 사실이며 최종 명령·결과는 저장소 `progress.md`가 기준입니다. 기존 `0.1.3` 범위의 amd64 runtime M1/M2, public `0.2.0` browser renderer와 public `0.2.1` 최소권한 인증 경로의 image/MCP smoke, `0.2.2` 관리형 인증 후보의 최종 local regression은 PASS이지만 실제 HAOS dashboard 완료까지 PASS라는 뜻은 아닙니다. 현재 공개 사전 릴리스는 `0.2.1`이고 `0.2.2`는 아직 공개되지 않은 후보입니다. 기존 설치의 실제 Supervisor 일반 업데이트와 HAOS browser 경로는 별도 확인합니다. Supervisor/Core/App lifecycle 실동작과 실제 물리 장치 제어는 운영 중단·오작동 위험 때문에 자동 완료 시험에서 제외합니다. Home Assistant `stage`는 M3 평가 전까지 `experimental`을 유지합니다.
