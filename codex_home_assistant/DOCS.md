...
> 현재 공개 사전 릴리스 `0.2.0`과 이 소스의 `0.2.1` 후보는 amd64 전용이며 Home Assistant `stage: experimental`을 유지합니다. `0.2.1`의 최소권한 browser 인증은 local container smoke와 실제 HAOS 실기 결과를 분리해 기록합니다. `/config` 전체를 쓰고 Home Assistant Core API와 Supervisor `manager` API를 호출할 수 있는 강한 관리자 도구이므로 인터넷에 SSH 포트를 직접 공개하지 마세요.
...
`0.2.1` image는 Node.js, `@playwright/mcp 0.0.78`, Alpine `chromium-headless-shell`과 한글·emoji font를 함께 제공합니다. Codex는 image-owned `/etc/codex/config.toml`의 `playwright` MCP server를 새 세션에서 자동으로 읽습니다. 사용자가 npm package나 browser binary를 설치하거나 `/data/codex/config.toml`에 MCP 명령을 복사할 필요가 없습니다. 이미 열려 있던 Codex 세션은 App 업데이트 후 자동으로 도구 목록을 다시 읽지 않으므로 새 세션을 시작하세요.
...
`8099` gateway는 frontend document, `/auth/`, `/api/`와 `/api/websocket`을 모두 내부 `homeassistant:<Core frontend port>`로 직접 전달합니다. App init은 Supervisor Core info에서 frontend port와 HTTP/HTTPS 여부를 찾고, 찾지 못하면 내부 기본값 `http://homeassistant:8123`을 사용합니다. direct Core 한 경로를 사용하므로 전용 사용자의 permission이 document·REST·WebSocket 전체에 동일하게 적용됩니다. gateway는 X-Forwarded-For, X-Real-IP와 Forwarded를 제거하고 App 밖에는 expose하지 않습니다.
...
### 전용 read-only browser 사용자 설정

Dashboard 자동 로그인은 optional `home_assistant_browser_token`이 다음 조건을 모두 만족할 때만 켜집니다.
...
App Web terminal에서 지원되는 Home Assistant admin WebSocket API로 전용 사용자를 만듭니다. 명령은 비밀번호를 두 번 묻고 화면에 표시하지 않으며 CLI 인수·환경변수·로그로 전달하지 않습니다.
...
token을 취소하거나 만료시키면 App option을 지우거나 새 token으로 바꾸고 재시작하세요. `system-read-only`도 모든 entity state를 읽을 수 있으며 특정 dashboard 하나만으로 제한된 권한은 아닙니다. App option과 backup, screenshot, snapshot, console/network 결과를 모두 민감자료로 취급하세요.
...
Local `0.2.1` 후보 image는 exact read-only user 재검증과 direct Core 경로를 포함한 full Docker smoke를 통과했습니다. `http://127.0.0.1:8099` fixture에서 desktop/mobile PNG를 실제 저장하고 console/network, REST/WebSocket과 Chromium→Core source IP `10.253.214.3`을 확인했습니다. 같은 IP를 replacement container에 재할당해도 접근이 거부됐고, validated token file 없이 환경의 `HA_BROWSER_TOKEN`만 주입한 browser는 인증되지 않았습니다. Public `0.2.0` → candidate update에서도 `/data/options.json`의 마스킹된 browser option과 기존 영속 데이터를 보존했습니다.
...
1. `0.2.1` image인지 확인하고 이미 실행 중이던 Codex를 종료한 뒤 새 세션을 시작합니다. `codex mcp list`에서 system server `playwright`가 enabled 상태인지 확인하되, token이나 전체 환경변수는 출력하지 않습니다.
...
4. Home Assistant dashboard라면 `ha-browser-network-info`와 `ha-browser-auth-status`를 확인한 다음 App shell에서 `http://127.0.0.1:8099`가 응답하는지 봅니다. browser token이나 `SUPERVISOR_TOKEN` 값을 직접 출력하거나 URL/query에 붙이지 마세요.
...
`home_assistant_browser_token` 노출이 의심되면 전용 사용자의 profile에서 해당 long-lived token을 즉시 삭제하고 App option도 지운 뒤 재시작합니다. `ha-browser-auth-status`가 `unconfigured` 또는 `rejected`이고 `8099`가 login page로 fail closed되는지 확인한 다음 새 token이 필요하면 다시 발급합니다. read-only token도 모든 entity state를 읽을 수 있으므로 화면 정보 유출로 취급하세요.
...
위 상태는 이 문서 작성 시점의 사실이며 최종 명령·결과는 저장소 `progress.md`가 기준입니다. 기존 `0.1.3` 범위의 amd64 runtime M1/M2, `0.2.0` browser renderer의 public image/MCP smoke와 `0.2.1` 최소권한 인증 후보의 local integration은 PASS이지만, 실제 HAOS dashboard 완료까지 PASS라는 뜻은 아닙니다. `0.2.0` GHCR image의 public pull은 확인했으며 `0.2.1`은 아직 공개되지 않은 후보입니다. 기존 설치의 실제 Supervisor 일반 업데이트와 HAOS browser 경로는 별도 확인합니다. Supervisor/Core/App lifecycle 실동작과 실제 물리 장치 제어는 운영 중단·오작동 위험 때문에 자동 완료 시험에서 제외합니다. Home Assistant `stage`는 M3 평가 전까지 `experimental`을 유지합니다.
