...
> 현재 공개 사전 릴리스 `0.2.2`와 이 소스의 `0.2.3` 후보는 amd64 전용이며 Home Assistant `stage: experimental`을 유지합니다. 관리형 최소권한 browser 인증, 기본 ON 자동화와 선택형 user-file refresh는 local container smoke와 실제 Home Assistant UI/HAOS 실기 결과를 분리해 기록합니다. `/config` 전체를 쓰고 Home Assistant Core API와 Supervisor `manager` API를 호출할 수 있는 강한 관리자 도구이므로 인터넷에 SSH 포트를 직접 공개하지 마세요.
...
| `0.2.3` 선택형 Codex 사용자 파일 갱신 | **후보 자동 검증은 `progress.md` 기준 / 실제 HAOS 구성 UI·Supervisor update는 NOT RUN** |
