...
> `0.1.3`은 amd64 전용 non-dev 릴리스이며 Home Assistant `stage: experimental`을 유지합니다. `/config` 전체를 쓰고 Home Assistant Core API와 Supervisor `manager` API를 호출할 수 있는 강한 관리자 도구입니다. 인터넷에 SSH 포트를 직접 공개하지 마세요.
...
`config.yaml`은 public generic manifest `ghcr.io/kanu-coffee/codex-for-home-assistant`를 사용합니다. Supervisor는 App version과 같은 `0.1.3` tag의 미리 빌드된 amd64 이미지를 내려받으므로 설치 장치에서 소스 빌드를 하지 않습니다.
...
`0.1.3-dev`에서 `0.1.3`으로 바뀌어도 `/data` 형식이나 영구 파일은 변경되지 않습니다. App Store 저장소를 새로고침한 뒤 일반 업데이트하고, App 완전 삭제·재설치나 `/data` 초기화는 하지 마세요. 업데이트 동안 Web UI/tmux와 SSH 연결이 잠시 종료되는 것은 정상입니다. 빈 `/data`의 최초 provisioning 자체를 다시 시험할 때만 별도의 완전 재설치 시험이 필요합니다.
...
위 상태는 이 문서 작성 시점의 사실이며 최종 명령·결과는 저장소 `progress.md`가 기준입니다. amd64 runtime의 M1/M2 실기 수용 기준은 PASS입니다. `0.1.3` GHCR image의 public pull과 기존 설치의 일반 업데이트 경로는 릴리스 검증으로 별도 확인합니다. Supervisor/Core/App lifecycle 실동작과 실제 물리 장치 제어는 운영 중단·오작동 위험 때문에 자동 완료 시험에서 제외합니다. Home Assistant `stage`는 M3 평가 전까지 `experimental`을 유지합니다.
