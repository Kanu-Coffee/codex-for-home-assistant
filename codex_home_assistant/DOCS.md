...
> 현재 공개 사전 릴리스는 `0.3.1`이며 amd64 전용 Home Assistant `stage: experimental`을 유지합니다. 정확한 공개 이미지 자동 회귀는 PASS했습니다. 제공된 실제 HAOS `0.3.0` read-only 감사의 catalog refresh는 FAIL했고, unavailable automation의 legal null config와 안전 진단을 보완한 공개 `0.3.1`의 실제 HAOS 재시험은 **NOT RUN**입니다. `/config` 전체를 쓰고 Home Assistant Core API와 Supervisor `manager` API를 호출할 수 있는 강한 관리자 도구이므로 인터넷에 SSH 포트를 직접 공개하지 마세요.
...
`config.yaml`은 public generic manifest `ghcr.io/kanu-coffee/codex-for-home-assistant`를 사용합니다. Supervisor는 App version과 같은 공개 `0.3.1` tag의 미리 빌드된 amd64 이미지를 내려받으므로 설치 장치에서 소스 빌드를 하지 않습니다.
...
`0.3.1`로 App version이 바뀌면 target별 1회 규칙은 다시 적용됩니다. 현재 `refresh_agents` 또는 `refresh_all`을 계속 선택했다면 선택 target이 `0.3.1`에서 한 번 다시 갱신됩니다. 이 재적용을 원하지 않으면 **업데이트 전** App **구성**에서 `codex_user_files_update_mode: preserve`로 저장하세요.
...
첫 성공 refresh는 Core WebSocket의 area/device/entity registry, `get_states`, `automation/config`, `search/related`를 결합합니다. Automation은 entity registry와 state의 합집합으로 발견하며 state가 없는 disabled registry automation은 제한된 registry metadata로도 index합니다. Core가 unavailable automation에 성공 응답으로 `config: null`을 주면 빈 config와 bounded warning으로 수용하고 entity와 `search/related` graph는 유지합니다. 실제 command/envelope/related 실패는 partial snapshot으로 commit하지 않습니다. 다음 allowlist만 보존합니다.
...
Public `0.3.1`의 정확한 이미지에서 memory lifecycle/privacy/MCP/persistence와 실제 installed `ws`, 관리형 user/token 생성·재사용·회전·제거, user-file update, browser/gateway/Core WebSocket/ttyd/SSH와 public `0.3.0` → `0.3.1` update smoke를 확인했습니다. 동일 merge SHA의 native Linux [main CI 29391446446](https://github.com/Kanu-Coffee/codex-for-home-assistant/actions/runs/29391446446)도 전체 회귀를 통과했습니다. 공식 [Builder 29391579529](https://github.com/Kanu-Coffee/codex-for-home-assistant/actions/runs/29391579529)이 게시한 generic/per-arch index digest는 `sha256:6e726ad1ce95714fb4d4f29dcbb51f762cdd3e7b3eeb9c1be7edcbb6ff1c0126`, runtime manifest digest는 `sha256:175c1a2b4d45897f713ab37f71c96b96c9b182e2f74340843c18aa34e7d84954`이며 익명 pull, linux/amd64, version/arch/source label과 mutable `latest` 부재를 확인했습니다. 공개 `0.3.1`의 실제 HAOS Core registry/restart/candidate/change/privacy E2E는 **NOT RUN**입니다.

이 자동 smoke는 fixture page 검증 증거입니다. 별도로 사용자가 public `0.2.3`의 AppArmor 활성 상태 실제 HAOS에서 인증된 `8099` dashboard의 desktop/mobile 렌더링, console, network/정적 resource, Core WebSocket 경로를 **PASS**로 확인했습니다. 상세 실행 로그와 HAOS 버전은 제공되지 않아 추정하거나 저장소 증거로 기록하지 않습니다. 따라서 fixture의 세부 보안 음성 테스트를 실제 HAOS에서 모두 재현했다고 확대하지는 않습니다.
...
| Public `0.3.0` → public `0.3.1` memory refresh patch | **PASS — 익명 linux/amd64 resolve·pull, legal null config/installed `ws`/closed diagnostics, memory/full/managed-auth/user-file/update 공개 이미지 회귀; 실제 HAOS 0.3.1 재시험은 NOT RUN** |
...
위 상태는 이 문서 작성 시점의 사실이며 최종 명령·결과는 저장소 `progress.md`가 기준입니다. 기존 `0.1.3` 범위의 amd64 runtime M1/M2와 public `0.2.0` browser renderer, `0.2.1` 최소권한 경로, `0.2.2` 관리형 인증, public `0.2.3` 자동화·user-file refresh, `0.2.4` validation/evidence, public `0.3.0`과 `0.3.1` 자동 회귀는 PASS입니다. 기존 설치의 Home Assistant 구성 UI/Supervisor 일반 업데이트와 AppArmor 활성 상태의 인증된 `8099` dashboard 경로는 public `0.2.3`에서 사용자 확인 PASS입니다. 현재 공개 사전 릴리스는 `0.3.1`입니다. 0.3.0 memory bootstrap 실기는 FAIL했고 공개 0.3.1의 catalog/restart/candidate/change/privacy 실제 HAOS E2E는 아직 실행하지 않았습니다. Supervisor/Core/App lifecycle 실동작, 실제 물리 장치 제어와 다른 별도 미실행 항목은 운영 중단·오작동 위험 때문에 자동 완료 시험에서 제외합니다. Home Assistant `stage`는 M3 평가 전까지 `experimental`을 유지합니다.
