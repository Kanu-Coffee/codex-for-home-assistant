...
> 현재 공개 사전 릴리스 `0.3.0`은 amd64 전용이며 Home Assistant `stage: experimental`을 유지합니다. 검증 기반 HA 구조·사용자 의미 메모리, bounded MCP/CLI 검색, 변경 후 fresh API 검증과 충돌·이력·rollback을 추가합니다. 정확한 공개 이미지 자동 회귀는 PASS지만 새 메모리의 실제 HAOS Core registry/restart/update E2E는 **NOT RUN**입니다. `/config` 전체를 쓰고 Home Assistant Core API와 Supervisor `manager` API를 호출할 수 있는 강한 관리자 도구이므로 인터넷에 SSH 포트를 직접 공개하지 마세요.
...
`0.3.0`으로 App version이 바뀌므로 target별 1회 규칙은 다시 적용됩니다. 현재 `refresh_agents` 또는 `refresh_all`을 계속 선택했다면 선택 target이 `0.3.0`에서 한 번 다시 갱신됩니다. 이 재적용을 원하지 않으면 **업데이트 전** App **구성**에서 `codex_user_files_update_mode: preserve`로 저장하세요.
