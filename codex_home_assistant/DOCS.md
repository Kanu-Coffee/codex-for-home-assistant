...
현재 문서는 공개 앱 버전 `0.5.0`을 기준으로 합니다.
...
- 공개키 전용 SSH와 Codex 데스크톱 SSH 프로젝트
...
2. 출력된 `ssh-ed25519 ...` **공개키 한 줄**을 `authorized_keys`에 추가하고 앱을 재시작합니다. `id_ed25519` 개인키는 절대 복사하지 마세요.
...
이 기능은 휴대폰이 HAOS 앱에 직접 연결되는 방식이 아닙니다.

```text
ChatGPT 모바일 앱
  → 같은 계정·workspace의 Mac/Windows 데스크톱 앱
  → 공개키 SSH
  → Codex for Home Assistant의 /config
```

1. 위 SSH 설정을 완료하고 데스크톱 PC에서 `ssh codex-ha`가 성공하는지 확인합니다.
2. 원격 shell에서 `codex --version`과 로그인 상태를 확인합니다.
3. ChatGPT 데스크톱 앱의 **Settings → Connections → SSH**에서 `codex-ha`를 추가하거나 활성화합니다.
4. 원격 프로젝트 폴더로 `/config`를 선택합니다.
5. 데스크톱 앱에서 **Set up Remote**를 시작하고 휴대폰으로 QR code를 스캔합니다.
6. 휴대폰과 데스크톱에서 같은 ChatGPT 계정과 workspace를 확인하고 필요한 MFA/SSO 단계를 완료합니다.

모바일에서는 새 작업 시작, 기존 작업 계속, 후속 지시, 승인, diff·test·terminal 결과 확인이 가능합니다. 데스크톱 호스트는 깨어 있고 온라인이며 ChatGPT 앱이 실행 중이어야 합니다. Remote 제공 여부는 플랜, 지역, workspace 정책과 앱 버전에 따라 달라질 수 있습니다. 최신 절차는 OpenAI의 [Remote connections](https://learn.chatgpt.com/docs/remote-connections)를 확인하세요.
...
이 기능은 모델 자체가 스스로 학습하거나 승인 없이 집을 운영한다는 뜻이 아닙니다. 현재 `0.5.0`은 experimental이며 실제 HAOS의 자연어 기억→새 작업 회상 전체 흐름에는 아직 공개 검증 공백이 있습니다.
...
일반 업데이트는 `/data`의 Codex 인증·설정, SSH host key와 검증형 memory를 보존하도록 설계되어 있습니다. `codex_user_files_update_mode: preserve`가 기본값입니다. `refresh_agents` 또는 `refresh_all`을 유지한 채 다음 버전으로 올리면 선택한 파일이 새 버전에서 한 번 다시 갱신되므로, 일회성 갱신 뒤에는 `preserve`로 되돌리세요.
...
- 실제 HAOS의 `0.5.0` 자연어 memory 폐루프에는 공개 검증 공백이 있습니다.
