...
> `0.1.1-dev`는 amd64 전용 실험 개발판입니다. `/config` 전체를 쓰고 Home Assistant Core API와 Supervisor `manager` API를 호출할 수 있는 강한 관리자 도구입니다. 인터넷에 SSH 포트를 직접 공개하지 마세요.
...
| 기능 | Endpoint/helper | 0.1.1-dev 상태 |
| --- | --- | --- |
...
| ttyd/tmux TERM 수정과 shell 명령 입력 | **PASS (로컬 실제 WebSocket/Chrome), HAOS 재검증 필요** |
