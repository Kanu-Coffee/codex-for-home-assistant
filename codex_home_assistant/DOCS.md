...
`config.yaml`은 public generic manifest `ghcr.io/kanu-coffee/codex-for-home-assistant`를 사용합니다. Supervisor는 App version과 같은 공개 `0.2.3` tag의 미리 빌드된 amd64 이미지를 내려받으므로 설치 장치에서 소스 빌드를 하지 않습니다.
...
Public `0.2.3`의 관리형 user/token 생성·재사용·회전·제거, 강제 종료 임시파일 회수, 응답 유실·지속 cleanup 실패 복구, App replacement와 동시 실행 차단, Core/provider 장애 및 정책 변조 fail-closed를 fixture에서 확인했습니다. 같은 public image에서 option 누락/default ON 자동 생성, restart 재사용, OFF 상태의 runtime 차단·identity 보존, ON 재활성화, 수동 override 억제와 model-visible 8099 routing도 검증했습니다. `http://127.0.0.1:8099` fixture의 desktop/mobile PNG, console/network와 direct Core REST/WebSocket 회귀도 PASS입니다. 공식 Builder run `29334867268`이 게시한 generic/per-arch index digest는 `sha256:a53c7b2006301826a52bc9d9dc3c3ec8fd5e99d73b59a028b16c78bf7628d2a1`이며 익명 pull과 label을 확인했습니다.

이 local smoke는 fixture page 자동 검증 증거입니다. 별도로 사용자가 AppArmor 활성 상태의 실제 HAOS에서 인증된 `8099` dashboard의 desktop/mobile 렌더링, console, network/정적 resource, Core WebSocket 경로를 **PASS**로 확인했습니다. 상세 실행 로그와 HAOS 버전은 제공되지 않아 추정하거나 저장소 증거로 기록하지 않습니다. 따라서 fixture의 세부 보안 음성 테스트를 실제 HAOS에서 모두 재현했다고 확대하지는 않습니다.
...
위 상태는 이 문서 작성 시점의 사실이며 최종 명령·결과는 저장소 `progress.md`가 기준입니다. 기존 `0.1.3` 범위의 amd64 runtime M1/M2와 public `0.2.0` browser renderer, `0.2.1` 최소권한 경로, `0.2.2` 관리형 인증 및 public `0.2.3` 자동화·user-file refresh regression은 PASS입니다. 기존 설치의 Home Assistant 구성 UI/Supervisor 일반 업데이트와 AppArmor 활성 상태의 인증된 `8099` dashboard desktop/mobile·console·network/resource·WebSocket 경로도 사용자 확인 PASS입니다. 현재 공개 사전 릴리스는 `0.2.3`이고 이 소스는 `0.2.4` 릴리스 후보입니다. 새 실기 확인의 상세 로그와 HAOS 버전은 제공되지 않아 기록하지 않았습니다. Supervisor/Core/App lifecycle 실동작, 실제 물리 장치 제어와 다른 별도 미실행 항목은 운영 중단·오작동 위험 때문에 자동 완료 시험에서 제외합니다. Home Assistant `stage`는 M3 평가 전까지 `experimental`을 유지합니다.
