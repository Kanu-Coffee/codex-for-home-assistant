...
> `0.1.2-dev`는 amd64 전용 실험 개발판입니다. `/config` 전체를 쓰고 Home Assistant Core API와 Supervisor `manager` API를 호출할 수 있는 강한 관리자 도구입니다. 인터넷에 SSH 포트를 직접 공개하지 마세요.
...
인증은 `/data/codex/auth.json`에 저장되며 App 재시작 뒤에도 유지되도록 설계되었습니다. HAOS Web UI에서 인증된 Codex 실행은 확인됐지만 로그인 방식과 App 재시작/업데이트 후 영속성은 아직 M2 실기 검증 항목입니다.
...
| Core 로그 | `ha-core-logs` (`GET /core/logs`) | PASS — Core 2026.7.1 실기 진단 |
...
추가로 `/supervisor/logs`, `/host/info`, `/os/info`, App 목록 조회가 성공했습니다. 이는 조회·로그·설정 검사 범위의 증거이며 Supervisor start/stop/restart 전체나 Core REST `/states`·서비스 호출 성공을 의미하지 않습니다.
...
| resize, 브라우저 종료 후 tmux 재접속 | **M2 미검증** |
| 인증된 Codex 실행 | **PASS — HAOS Web UI 사용자 실기 확인** |
| Device code 방식과 App 재시작/업데이트 후 인증 영속성 | **M2 미검증** |
...
| Core REST 상태 조회와 안전한 entity 서비스 호출 | **M2 미검증** |
| Supervisor 정보/로그 조회 | **PASS — 일부 manager endpoint** |
| Supervisor/Core/App start/stop/restart 운영 | **M2 미검증** |
| 기본 전역 운영 지침 생성·사용자 override 보존 | **PASS (로컬 컨테이너), HAOS 0.1.2-dev 재검증 필요** |
