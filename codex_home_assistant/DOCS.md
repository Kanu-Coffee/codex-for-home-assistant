...
| Core 로그 helper | `ha-core-logs` | `0.1.2-dev` FAIL 원인 수정, `0.1.3-dev` HAOS 재검증 필요 |
...
| App 로그 helper | `ha-addon-logs self` | `0.1.2-dev` FAIL 원인 수정, `0.1.3-dev` HAOS 재검증 필요 |
...
| resize, 브라우저 종료 후 같은 tmux pane 재접속 | **PASS (실제 ttyd WebSocket 로컬 smoke), HAOS UI 재검증 필요** |
...
| `0.1.3-dev` Core/App 로그 helper | **M2 업데이트 재검증 필요** |
