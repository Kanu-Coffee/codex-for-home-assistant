...
> 현재 공개 사전 릴리스는 `0.3.1`이며 amd64 전용 `experimental` 단계입니다. 정확한 공개 이미지 자동 회귀는 PASS했습니다. 제공된 실제 HAOS `0.3.0` 읽기 전용 감사의 memory catalog refresh는 FAIL했고, legal unavailable-automation null config와 안전 진단을 보완한 공개 `0.3.1`의 실제 HAOS 재시험은 아직 **NOT RUN**입니다. 실제 장치 제어와 Core/App lifecycle 변경은 안전상 자동 수용 시험에서 제외합니다.
