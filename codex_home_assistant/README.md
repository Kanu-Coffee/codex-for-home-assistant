...
> 다음 릴리스 대상은 `0.2.0`, amd64 전용이며 `experimental` 단계입니다. 기존 HAOS 자동 시작, Web UI, Codex 로그인·재시작 인증 보존, Core/Supervisor API, 공개키 SSH와 host identity 보존은 실기 확인했습니다. 새 browser renderer와 Home Assistant dashboard gateway는 아직 public release 및 HAOS 실기 검증 전입니다. 실제 장치 제어와 Core/App lifecycle 변경은 안전상 자동 수용 시험에서 제외합니다.
...
`0.2.0` 공개 후 Supervisor는 public GHCR에서 같은 version의 amd64 image를 내려받습니다. 설치 장치에서 Dockerfile을 소스 빌드하지 않습니다. 공개 전에는 `0.2.0` tag가 설치 가능하다고 가정하지 마세요.
