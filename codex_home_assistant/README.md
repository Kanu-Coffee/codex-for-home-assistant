...
> 현재 버전은 `0.1.3`, amd64 전용이며 `experimental` 단계입니다. HAOS 자동 시작, Web UI, Codex 로그인·재시작 인증 보존, Core/Supervisor API, 공개키 SSH와 host identity 보존을 실기 확인했습니다. 실제 장치 제어와 Core/App lifecycle 변경은 안전상 자동 수용 시험에서 제외합니다.
...
Supervisor는 public GHCR에서 `0.1.3` amd64 image를 내려받습니다. 설치 장치에서 Dockerfile을 소스 빌드하지 않습니다.

`0.1.3-dev`에서 `0.1.3`으로 바뀌어도 `/data` 마이그레이션은 없습니다. 기존 App을 완전 삭제하거나 초기화하지 말고 일반 업데이트하세요.
