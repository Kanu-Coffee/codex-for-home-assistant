...
> 현재 공개 사전 릴리스는 `0.2.2`, 이 소스의 다음 후보는 `0.2.3`이며 amd64 전용 `experimental` 단계입니다. 기존 HAOS 자동 시작, Web UI, Codex 로그인·재시작 인증 보존, Core/Supervisor API, 공개키 SSH와 host identity 보존은 실기 확인했습니다. 관리형 최소권한 browser 인증과 `0.2.3`의 기본 ON 자동화는 local container fixture로 검증하고 실제 HAOS/AppArmor dashboard E2E와 분리해 기록합니다. 실제 장치 제어와 Core/App lifecycle 변경은 안전상 자동 수용 시험에서 제외합니다.
...
Local `0.2.3` 후보 image는 option 누락/default ON 자동 생성, restart identity 재사용, OFF/ON 보존·재활성화, ON 삭제 거부·OFF 삭제와 수동 override 억제를 fixture에서 확인했습니다. model-visible `8099` 지침, 내부 gateway의 desktop/mobile PNG·console/network·direct Core REST/WebSocket, public `0.2.2` replacement smoke도 통과했습니다. 실제 HAOS dashboard/AppArmor 경로는 아직 **NOT RUN**입니다.
