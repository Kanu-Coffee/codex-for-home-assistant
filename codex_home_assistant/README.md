...
> 현재 공개 사전 릴리스는 `0.2.0`, amd64 전용이며 `experimental` 단계입니다. 기존 HAOS 자동 시작, Web UI, Codex 로그인·재시작 인증 보존, Core/Supervisor API, 공개키 SSH와 host identity 보존은 실기 확인했습니다. 새 browser renderer와 Home Assistant dashboard gateway의 public image smoke는 PASS지만 실제 HAOS 실기 검증은 아직 전입니다. 실제 장치 제어와 Core/App lifecycle 변경은 안전상 자동 수용 시험에서 제외합니다.
