...
> 현재 공개 사전 릴리스는 `0.3.2`이며 amd64 전용 `experimental` 단계입니다. 정확한 공개 이미지 자동 회귀는 PASS했습니다. 이전 public `0.3.1`의 실제 HAOS/Core `2026.7.2` 실기에서는 automation-related 30건 중 2건의 `unknown_error` 때문에 catalog가 FAIL했고 Core restart는 PARTIAL이었습니다. Public `0.3.2`는 이 오류를 격리했지만 실제 HAOS 재시험은 아직 **NOT RUN**입니다.
...
Supervisor는 public GHCR에서 현재 공개 version `0.3.2`의 amd64 image를 내려받습니다. 공개 tag의 인증 없는 pull과 공개 이미지 회귀, 동일 merge SHA의 native Linux 전체 컨테이너 smoke를 확인했으며 설치 장치에서 Dockerfile을 소스 빌드하지 않습니다.
...
결과 파일은 `/run/codex-ha/playwright-output`에 최대 50 MiB만 저장하고 임의 `filename`을 거부합니다. App 시작·재시작 때 기존 결과를 삭제하며 `/data`에는 보존하지 않습니다. exact token 문자열은 text output에서 마스킹되지만 인코딩·분할된 비밀, screenshot과 page data 전체를 정화하지는 않으므로 민감한 Home Assistant 정보가 나타날 수 있습니다. 클릭·입력은 실제 상태를 변경할 수 있으므로 별도 승인이 필요합니다. 공개 `0.3.2` amd64 image의 인증 없는 generic/per-arch resolve·pull, version/arch/source label과 자동 smoke는 **PASS**입니다. generic/per-arch OCI index digest는 `sha256:d14bb71190be15fa6a45a19a2e981cb173c741e37ab0344c9bf941abbaab2c6b`, runtime manifest digest는 `sha256:3af26175df8c35a83e19634a0f7278f3ca3f70b08cb035e9953f2a519a9f353a`이고 `latest`는 없습니다. 동일 merge SHA의 [main CI 29404524753](https://github.com/Kanu-Coffee/codex-for-home-assistant/actions/runs/29404524753)과 [Builder 29404702270](https://github.com/Kanu-Coffee/codex-for-home-assistant/actions/runs/29404702270)이 PASS했습니다. 별도 실제 HAOS `0.3.1` 실기에서는 catalog가 related `unknown_error`로 FAIL했고 Core restart는 PARTIAL, privacy는 PASS였습니다. Public `0.3.2`의 실제 HAOS 재시험은 아직 **NOT RUN**입니다.
...
App version별 1회 규칙은 public `0.3.2`에도 적용됩니다. `refresh_agents` 또는 `refresh_all`을 선택한 채로 업데이트하면 선택 target이 `0.3.2`에서 한 번 다시 갱신됩니다. 재적용을 원하지 않으면 **업데이트 전** Home Assistant App **구성**에서 `preserve`로 저장하세요.
