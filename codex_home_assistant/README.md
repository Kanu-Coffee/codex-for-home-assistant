...
> 현재 공개 사전 릴리스는 `0.3.0`, 다음 patch 후보는 `0.3.1`이며 amd64 전용 `experimental` 단계입니다. 0.3.0 자동 회귀는 PASS였지만 제공된 실제 HAOS 읽기 전용 감사의 memory catalog refresh는 FAIL했습니다. 0.3.1은 legal unavailable-automation null config, 고정 `ws` transport와 단계별 안전 진단을 보완하며 공개 image의 실제 HAOS 재시험은 아직 **NOT RUN**입니다. 실제 장치 제어와 Core/App lifecycle 변경은 안전상 자동 수용 시험에서 제외합니다.
...
Supervisor는 public GHCR에서 현재 공개 version `0.3.0`의 amd64 image를 내려받습니다. 공개 tag의 인증 없는 pull과 공개 이미지 회귀, 동일 merge SHA의 native Linux 전체 컨테이너 smoke를 확인했으며 설치 장치에서 Dockerfile을 소스 빌드하지 않습니다.
...
- `ha-memoryd`는 고정 Supervisor WebSocket proxy와 image-pinned `ws`로 catalog를 갱신하고, unavailable automation의 legal null config는 유지하며 실제 실패는 원문·token 없이 closed status code로 구분합니다.
...
결과 파일은 `/run/codex-ha/playwright-output`에 최대 50 MiB만 저장하고 임의 `filename`을 거부합니다. App 시작·재시작 때 기존 결과를 삭제하며 `/data`에는 보존하지 않습니다. exact token 문자열은 text output에서 마스킹되지만 인코딩·분할된 비밀, screenshot과 page data 전체를 정화하지는 않으므로 민감한 Home Assistant 정보가 나타날 수 있습니다. 클릭·입력은 실제 상태를 변경할 수 있으므로 별도 승인이 필요합니다. 공개 `0.3.0` amd64 image의 인증 없는 generic/per-arch resolve·pull, version/arch/source label과 자동 smoke는 **PASS**입니다. generic/per-arch OCI index digest는 `sha256:5ddb290534d45d46b34d2ef478ede83110c3b8e3a56f601ce2c6b45d5814341f`, runtime manifest digest는 `sha256:1c1e6a43cf24508e1c39c62bb2f0ab57c635208d8bd47537d4f9d09f0487f360`이고 `latest`는 없습니다. 동일 merge SHA의 [main CI 29384344027](https://github.com/Kanu-Coffee/codex-for-home-assistant/actions/runs/29384344027)과 [Builder 29384484760](https://github.com/Kanu-Coffee/codex-for-home-assistant/actions/runs/29384484760)이 PASS했습니다. 이후 실제 HAOS 0.3.0 read-only memory audit는 catalog FAIL이고 0.3.1 live 재시험은 NOT RUN입니다.
...
App version별 1회 규칙은 `0.3.1`에도 적용됩니다. `refresh_agents` 또는 `refresh_all`을 선택한 채로 업데이트하면 선택 target이 `0.3.1`에서 한 번 다시 갱신됩니다. 재적용을 원하지 않으면 **업데이트 전** Home Assistant App **구성**에서 `preserve`로 저장하세요.
...
공개 `0.3.0`의 browser 경로는 option 누락/default ON 자동 생성, restart identity 재사용, OFF/ON 보존·재활성화, ON 삭제 거부·OFF 삭제와 수동 override 억제를 fixture와 정확한 공개 이미지에서 확인했습니다. model-visible `8099` 지침, 내부 gateway의 desktop/mobile PNG·console/network·direct Core REST/WebSocket, 선택형 user-file refresh와 public `0.2.4` replacement smoke도 통과했습니다. 실제 Home Assistant 구성 UI/Supervisor 일반 업데이트와 HAOS AppArmor의 인증된 `8099` dashboard 경로는 public `0.2.3`에서 사용자 확인 **PASS**입니다. 별도로 0.3.0 memory catalog live audit는 FAIL했고 0.3.1은 unavailable automation `config: null`, fixed Supervisor proxy/pinned `ws`, closed diagnostic code와 endpoint-redirection 차단을 자동 검증한 뒤 실제 HAOS 재시험을 기다립니다.
