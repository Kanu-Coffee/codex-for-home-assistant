...
Supervisor는 public GHCR에서 현재 공개 version `0.4.0`의 amd64 image를 내려받습니다. 공개 tag의 인증 없는 pull과 공개 이미지 회귀, 동일 merge SHA의 native Linux 전체 컨테이너 smoke를 확인했으며 설치 장치에서 Dockerfile을 소스 빌드하지 않습니다.

기존 App은 완전 삭제하거나 초기화하지 말고 일반 업데이트하세요. Playwright MCP는 image-owned system config로 추가되므로 `/data` 마이그레이션이나 수동 MCP 등록이 필요하지 않습니다. Public `0.2.2`에서 `0.2.3`으로 올린 첫 시작은 새 user-file option이 없어 기본 `preserve`로 동작합니다. `0.4.0` 업데이트에서 새 `browser_approval_policy`가 없으면 기본 `safe`로 동작합니다. 업데이트 뒤 App과 Codex 세션을 다시 시작해야 새 정책과 도구를 읽습니다.
...
`0.4.0`의 **헤드리스 브라우저 승인 정책**은 `safe`(기본), `never`, `always`입니다. `safe`는 탐색·조회·스크린샷을 승인창 없이 실행하고 click·form·key·select·type에서 확인합니다. `never`는 현재 허용된 16개 도구에 대한 MCP 승인창을 생략하며 `always`는 모두 확인합니다. 금지된 arbitrary code/file upload 도구는 어느 mode에서도 활성화되지 않습니다. 최상위 `codex_approval_policy=never`는 전역 full-auto라 `safe`/`always`의 prompt보다 우선할 수 있습니다. 변경 뒤 App과 Codex 세션을 재시작하세요.
...
결과 파일은 `/run/codex-ha/playwright-output`에 최대 50 MiB만 저장하고 임의 `filename`을 거부합니다. App 시작·재시작 때 기존 결과를 삭제하며 `/data`에는 보존하지 않습니다. exact token 문자열은 text output에서 마스킹되지만 인코딩·분할된 비밀, screenshot과 page data 전체를 정화하지는 않으므로 민감한 Home Assistant 정보가 나타날 수 있습니다. 클릭·입력은 실제 상태를 변경할 수 있으므로 별도 승인이 필요합니다. 공개 [`0.4.0` Release](https://github.com/Kanu-Coffee/codex-for-home-assistant/releases/tag/0.4.0), 기능 merge [`bca6126`](https://github.com/Kanu-Coffee/codex-for-home-assistant/commit/bca612661692e3d66d239c06b57b52921ea56af6), [main CI 29408206017](https://github.com/Kanu-Coffee/codex-for-home-assistant/actions/runs/29408206017)과 [Builder 29408467932](https://github.com/Kanu-Coffee/codex-for-home-assistant/actions/runs/29408467932)을 확인했습니다. 인증 없는 generic/per-arch resolve·pull, linux/amd64와 version/arch/source label, mutable `latest` 부재가 PASS했으며 OCI index digest는 `sha256:758837276c4247a304c58791bddab5912977d3445801dcd832a638f9a2af9342`, runtime manifest digest는 `sha256:b586727e9a2ca724f32f8255f692cd32104aeed45bc0e65b8c12cb3cc151373b`입니다. 정확한 공개 이미지의 full browser/gateway/Core WebSocket/ttyd/SSH, browser approval policy, memory, managed-auth, user-file update와 public `0.3.2` → `0.4.0` update smoke가 **PASS**했습니다. 후속 실제 HAOS `never` mode에서는 14개 허용 도구가 승인 요청 0회로 PASS했고 desktop/mobile 자동 인증 dashboard와 비파괴 click/input 경로가 동작했습니다. `select_option`·`close`와 `safe`/`always`, 전역 `codex_approval_policy=never` precedence, 금지 도구, Configuration UI/default, AppArmor 활성 여부, user-file/identity 보존과 live update는 **NOT TESTED/NOT RUN**입니다.
...
App version별 1회 규칙은 public `0.4.0`에도 적용됩니다. `refresh_agents` 또는 `refresh_all`을 선택한 채로 업데이트하면 선택 target이 `0.4.0`에서 한 번 다시 갱신됩니다. 재적용을 원하지 않으면 **업데이트 전** Home Assistant App **구성**에서 `preserve`로 저장하세요.
