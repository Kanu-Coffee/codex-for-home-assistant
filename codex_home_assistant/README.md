...
> 현재 공개 사전 릴리스는 `0.2.4`이며 amd64 전용 `experimental` 단계입니다. `0.2.4`는 런타임 기능 변경 없이 public `0.2.3`의 HAOS 실기 완료 기록을 반영한 validation/evidence patch입니다. 기존 HAOS 자동 시작, Web UI, Codex 로그인·재시작 인증 보존, Core/Supervisor API, 공개키 SSH와 host identity 보존에 더해 public `0.2.3`에서 Home Assistant 구성 UI/Supervisor 일반 업데이트와 AppArmor 활성 상태의 인증된 `8099` dashboard desktop/mobile·console·network/resource·WebSocket 경로를 사용자가 **PASS**로 확인했습니다. 실제 장치 제어와 Core/App lifecycle 변경은 안전상 자동 수용 시험에서 제외합니다.
...
Supervisor는 public GHCR에서 현재 공개 version `0.2.4`의 amd64 image를 내려받습니다. 공개 tag의 인증 없는 pull과 공개 이미지 회귀, 동일 merge SHA의 native Linux 전체 컨테이너 smoke를 확인했으며 설치 장치에서 Dockerfile을 소스 빌드하지 않습니다.
...
결과 파일은 `/run/codex-ha/playwright-output`에 최대 50 MiB만 저장하고 임의 `filename`을 거부합니다. App 시작·재시작 때 기존 결과를 삭제하며 `/data`에는 보존하지 않습니다. exact token 문자열은 text output에서 마스킹되지만 인코딩·분할된 비밀, screenshot과 page data 전체를 정화하지는 않으므로 민감한 Home Assistant 정보가 나타날 수 있습니다. 클릭·입력은 실제 상태를 변경할 수 있으므로 별도 승인이 필요합니다. 공개 `0.2.4` amd64 image의 인증 없는 generic/per-arch resolve·pull, version/arch/source label, managed-auth/user-file recovery와 public `0.2.3` → `0.2.4` update/MCP smoke는 **PASS**입니다. generic/per-arch OCI index digest는 `sha256:3f9d70943ad68f2b7177826546227fa00c68068b57bdf7b1686edf1c3de90f26`, runtime manifest digest는 `sha256:e435844c06d7813ad527e3897dd647cc313a03d7c7c833301953b253a57658a8`이고 `latest`는 없습니다. 동일 merge SHA의 native Linux main CI에서 desktop `1440x900`·mobile `390x844` PNG, console/network, Core REST/WebSocket, ttyd/SSH와 token 비노출을 포함한 전체 컨테이너 smoke가 PASS했습니다. 현재 Alpine 3.24 + system Chromium 조합은 upstream Playwright의 공식 Alpine/musl 지원 대상이 아니며, 실제 Home Assistant 구성 UI/Supervisor 일반 업데이트와 AppArmor 활성 상태의 인증된 `8099` dashboard desktop/mobile·console·network/resource·WebSocket 경로는 public `0.2.3`에서 사용자 확인 **PASS**입니다.
...
`0.2.4`는 런타임 기능 변경 없는 validation/evidence patch이지만 App version별 1회 규칙은 그대로 적용됩니다. `refresh_agents` 또는 `refresh_all`을 선택한 채로 업데이트하면 선택 target이 `0.2.4`에서 한 번 다시 갱신됩니다. 재적용을 원하지 않으면 **업데이트 전** Home Assistant App **구성**에서 `preserve`로 저장하세요.
...
공개 `0.2.4`의 browser 경로는 option 누락/default ON 자동 생성, restart identity 재사용, OFF/ON 보존·재활성화, ON 삭제 거부·OFF 삭제와 수동 override 억제를 fixture에서 확인했습니다. model-visible `8099` 지침, 내부 gateway의 desktop/mobile PNG·console/network·direct Core REST/WebSocket, 선택형 user-file refresh와 public `0.2.3` replacement smoke도 통과했습니다. 실제 Home Assistant 구성 UI/Supervisor 일반 업데이트와 HAOS AppArmor의 인증된 `8099` dashboard desktop/mobile·console·network/resource·WebSocket 경로는 public `0.2.3`에서 사용자 확인 **PASS**입니다. 상세 로그와 HAOS 버전은 제공되지 않아 기록하지 않습니다.
