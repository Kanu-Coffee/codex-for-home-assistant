...
> 현재 공개 사전 릴리스는 `0.2.0`, 이 소스의 다음 후보는 `0.2.1`이며 amd64 전용 `experimental` 단계입니다. 기존 HAOS 자동 시작, Web UI, Codex 로그인·재시작 인증 보존, Core/Supervisor API, 공개키 SSH와 host identity 보존은 실기 확인했습니다. 새 최소권한 browser 인증의 실제 HAOS 실기 검증은 아직 전입니다. 실제 장치 제어와 Core/App lifecycle 변경은 안전상 자동 수용 시험에서 제외합니다.
...
Supervisor는 public GHCR에서 `0.2.0`과 같은 version의 amd64 image를 내려받습니다. 공개 tag의 인증 없는 pull과 전체 컨테이너 smoke를 확인했으며 설치 장치에서 Dockerfile을 소스 빌드하지 않습니다.

기존 App은 완전 삭제하거나 초기화하지 말고 `0.2.0`으로 일반 업데이트하세요. Playwright MCP는 image-owned system config로 추가되므로 `/data` 마이그레이션이나 수동 MCP 등록이 필요하지 않습니다. 업데이트 뒤 새 Codex 세션을 시작해야 새 도구를 읽습니다.
...
`0.2.1` 후보에서 dashboard 자동 로그인은 마스킹된 `home_assistant_browser_token` 옵션이 App 시작과 각 MCP 시작에 정확히 active·local-only·sole `system-read-only` user로 검증될 때만 켜집니다. 검증에만 사용한 Supervisor credential은 Node proxy와 Chromium 실행 전에 제거하고, Supervisor token fallback은 제공하지 않습니다. 미설정·검증 실패 시 login page가 보입니다. `ha-browser-network-info`로 확인하는 현재 App `/32`는 동적 주소라 `trusted_networks` 또는 `trusted_proxies`에 넣지 않습니다. Home Assistant `configuration.yaml`, `.storage`와 기존 `homeassistant` provider는 수정하지 않습니다.
...
Local `0.2.1` 후보는 내부 `8099` fixture의 desktop/mobile PNG, console/network, direct Core REST/WebSocket과 App/Core/Chromium source IP 일치, stale IP·상속 token 음성 테스트 및 public `0.2.0`에서의 영속 option 보존까지 full smoke를 통과했습니다. 실제 HAOS dashboard와 AppArmor 경로는 계속 **NOT RUN**입니다.
