...
> 현재 공개 사전 릴리스는 `0.3.0`이며 amd64 전용 `experimental` 단계입니다. 검증 기반 HA 구조·사용자 의미 메모리와 bounded MCP/CLI 검색, 변경 후 fresh API 검증, 충돌·이력·rollback을 제공합니다. 정확한 공개 이미지의 자동 회귀는 PASS지만 새 메모리의 실제 HAOS Core registry/restart/update E2E는 **NOT RUN**입니다. 실제 장치 제어와 Core/App lifecycle 변경은 안전상 자동 수용 시험에서 제외합니다.
...
결과 파일은 `/run/codex-ha/playwright-output`에 최대 50 MiB만 저장하고 임의 `filename`을 거부합니다. App 시작·재시작 때 기존 결과를 삭제하며 `/data`에는 보존하지 않습니다. exact token 문자열은 text output에서 마스킹되지만 인코딩·분할된 비밀, screenshot과 page data 전체를 정화하지는 않으므로 민감한 Home Assistant 정보가 나타날 수 있습니다. 클릭·입력은 실제 상태를 변경할 수 있으므로 별도 승인이 필요합니다. 공개 `0.3.0` amd64 image의 인증 없는 generic/per-arch resolve·pull, version/arch/source label, memory/full browser, managed-auth/user-file recovery와 public `0.2.4` → `0.3.0` update smoke는 **PASS**입니다. generic/per-arch OCI index digest는 `sha256:5ddb290534d45d46b34d2ef478ede83110c3b8e3a56f601ce2c6b45d5814341f`, runtime manifest digest는 `sha256:1c1e6a43cf24508e1c39c62bb2f0ab57c635208d8bd47537d4f9d09f0487f360`이고 `latest`는 없습니다. 동일 merge SHA의 native Linux [main CI 29384344027](https://github.com/Kanu-Coffee/codex-for-home-assistant/actions/runs/29384344027)과 [Builder 29384484760](https://github.com/Kanu-Coffee/codex-for-home-assistant/actions/runs/29384484760)이 PASS했습니다. 현재 Alpine 3.24 + system Chromium 조합은 upstream Playwright의 공식 Alpine/musl 지원 대상이 아니며, 새 메모리의 실제 HAOS E2E는 **NOT RUN**입니다.
...
App version별 1회 규칙은 `0.3.0`에도 적용됩니다. `refresh_agents` 또는 `refresh_all`을 선택한 채로 업데이트하면 선택 target이 `0.3.0`에서 한 번 다시 갱신됩니다. 재적용을 원하지 않으면 **업데이트 전** Home Assistant App **구성**에서 `preserve`로 저장하세요.
