...
- `codex_approval_policy=never`는 Codex 전체 full-auto로 MCP prompt를 자동 승인할 수 있어 browser `safe`/`always`보다 상위에서 작동한다. browser option은 MCP popup 제어이며 proxy allowlist, 사용자 요청의 권한 범위나 Home Assistant 기기 안전 지침을 확장하지 않는다.
