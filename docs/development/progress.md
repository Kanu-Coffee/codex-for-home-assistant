...
- 상태: **public 0.6.0 amd64 정확 이미지 전체 회귀 PASS / DEV 0.7.0-dev.1 `Codex for Home Assistant (DEV)` amd64+aarch64·민감 경로·공급망 후보 구현·post-bump local amd64 image/smoke PASS / dev push 완료·CI 31547358759/PR CI 31547993600 managed sandbox FAIL·CI-only pinned bwrap AppArmor profile 수정 local PASS / Builder 31547993837 permission graph FAIL·caller-permission inheritance 수정 local PASS / GHCR publish·aarch64 HAOS·새 AppArmor HAOS 실기 NOT RUN / 실제 HAOS feedback live NOT RUN / 실제 HAOS `never` 14/16 승인 0회 PASS·전체 승인 행렬 PARTIAL / public 0.3.2 memory live PARTIAL(FAIL 0) / 0.5.0 자연어 memory live 수용 대기**
