...
- 상태: **public 0.6.0 amd64 정확 이미지 전체 회귀 PASS / DEV 0.7.0-dev.1 `Codex for Home Assistant (DEV)` amd64+aarch64·민감 경로·공급망 후보 구현·post-bump local amd64 image/smoke PASS / dev push 완료·CI 31547358759 managed sandbox FAIL·test-container SETFCAP 수정 local PASS / GHCR publish·aarch64 HAOS·새 AppArmor HAOS 실기 NOT RUN / 실제 HAOS feedback live NOT RUN / 실제 HAOS `never` 14/16 승인 0회 PASS·전체 승인 행렬 PARTIAL / public 0.3.2 memory live PARTIAL(FAIL 0) / 0.5.0 자연어 memory live 수용 대기**
- 현재 마일스톤: **DEV 0.7.0-dev.1 멀티아키텍처·민감정보 경계 안정화 / CI 재검증·HAOS 실기·release 검증 대기**
...
- 공급망 발견: 후보 SBOM scan에서 shared privileged WebSocket runtime `ws` `8.18.3`의 fixable High `GHSA-96hv-2xvq-fx4p`를 확인해 현재 lockfile을 `8.21.0`으로 올렸다. 최종 local amd64 image를 Grype `0.110.0`으로 다시 검사해 Critical 0, High 72, Medium 31, Low 2, Unknown 1을 기록했다. Native aarch64 scan과 release attestation 실행은 아래 미검증 경계에 남긴다.
- Binary 후속: GitHub CLI를 `2.97.0`으로 올렸다. Grype `0.110`은 base에서 상속한 TempIO와 official `2026.07.0` 교체 후보 모두의 embedded `golang.org/x/crypto` `v0.31.0`에서 Critical 7건을 보고했다. Repository와 base init 경로에 사용처가 없어 final image에서 `/usr/bin/tempio`를 제거했고 최종 local amd64 scan에서 Critical 0과 executable 부재를 확인했다. Native aarch64와 실제 release workflow의 Critical gate는 아직 실행하지 않았다.
- local 통합 검증: Post-bump Python unit/contract **111 passed**, yamllint exit 0(기존 Issue Form line-length warning만 있음), Markdown 50 files·ShellCheck·Hadolint·actionlint·AppArmor parser·exact Home Assistant App linter와 `git diff --check` PASS를 확인했다. Post-bump amd64 image `codex-ha:dev-0.7.0-dev.1` build가 PASS했고 `io.hass.version`·OCI version이 `0.7.0-dev.1`, OCI title·MOTD가 `(DEV)`, baked app-version이 label과 일치했다. 이 최신 image에서 Docker/ttyd/SSH/Core API, user-file update, feedback(외부 write 없음), memory, browser approval, managed browser auth, managed Codex sandbox와 public `0.5.0` → candidate update smoke가 모두 PASS했다. Architecture metadata mismatch build는 양방향 contract에 고정했고 amd64 host에서 실제 aarch64 mismatch build가 fail closed함을 확인했다.
...
- CI 보완: Linux 5.12+의 nested root UID mapping에 필요한 `CAP_SETFCAP`을 disposable managed-sandbox test container에만 명시했다. Home Assistant App manifest와 runtime privilege는 바뀌지 않는다. Corrected local managed-sandbox smoke와 multiarch contract **23 passed**는 PASS했고 remote PR 재검증은 대기 중이다.
- 검증 경계: exact `0.7.0-dev.1` GHCR image·published multi-architecture manifest, native/release scan, 실제 aarch64 HAOS 설치·Ingress/SSH/Codex/API/browser, 실제 HAOS AppArmor/managed requirements와 release signing/attestation은 **NOT RUN**이다. Local Docker smoke는 AppArmor를 활성화하지 않으므로 구현·parser·정적 계약을 HAOS 실기 PASS로 확대하지 않는다.
- 전달 경계: commit `7eeb329e409a26c8601b7adeaa1d52023bc2c0d9`을 remote `dev`에 push했다. PR, merge, `0.7.0-dev.1` tag, GHCR publish와 GitHub release는 수행하지 않았고 `main`과 public `0.6.0` image는 변경하지 않았다.
...
- Known issues: 공식 명시 Linux 지원은 Ubuntu/Debian 중심이지만 amd64 Alpine/musl remote app server는 사용자 E2E에서 동작했다. aarch64 artifact와 CI 구성을 구현했지만 native runner와 실제 HAOS는 아직 검증하지 않았다. 향후 Codex 버전·base/architecture 변경과 HAOS lifecycle 영향도 별도 검증 대상이다.
...
- [ ] Native ARM runner job 실제 실행
- [ ] aarch64 HAOS 실기 테스트
- [x] `arch`에 aarch64 후보 추가 — public release 지원 표시는 native CI·HAOS 수용 뒤 확정
...
- [ ] Release workflow 실행에서 multi-arch manifest와 SBOM/scan/signing/attestation 검증
- [ ] 설치·복구 UX 개선
...
| ID | 질문 | 현재 처리 |
|---|---|---|
| Q-001 | Codex standalone 바이너리가 선택한 Alpine/base image의 amd64 및 aarch64에서 안정적인가? | amd64 CLI/app-server help와 public image 회귀 PASS; aarch64 official asset/checksum 매핑은 구현했으나 native CI·HAOS는 NOT RUN |
| Q-002 | ttyd의 Ingress base path/WebSocket 처리가 추가 플래그 없이 가능한가? | nginx/ttyd 로컬 PASS; 실제 Supervisor Ingress는 M2 |
| Q-003 | Codex `workspace-write` sandbox가 HAOS App 컨테이너에서 정상 작동하는가? | DEV `0.7.0-dev.1` 기본/강제값과 network access·managed requirements 구현; native/actual HAOS 통합 검증 NOT RUN |
