...
- interactive/ingress/browser S6 graph와 독립돼 주기적 `ha-memory refresh`를 실행하는 `ha-memoryd` longrun scheduler, retry/backoff와 status

테스트:

- clean first boot, 반복 v1 initialization, unsupported schema fail-closed와 restart/update persistence
...
- entity/device/area registry list, `get_states`, `automation/config`, `search/related` command allowlist
- object/relationship allowlist normalization, unavailable automation의 legal `config: null` → empty config/bounded warning 처리, complete snapshot staging과 atomic revision commit
...
- unavailable automation null-config 성공, unsupported command, 일부 automation 실패, malformed/oversized field와 transport interruption
...
- `change begin --subjects-json --expect-json`의 기존/생성 예정 subject, pre-change digest/field summary와 같은 계약을 사용하는 fresh API 기반 `change verify --expect-json`; `codex_change` relationship candidate는 동일 source·relation·target의 성공 predicate에만 연결
...
- search/show/propose/evidence/verify/apply/change/history/conflict/rollback/status tool schema와 search 32 KiB·다른 read row/field·MCP hard output limit
- history-preserving audit event/change, compensating rollback과 HA catalog rollback 금지
- 새 기본 `AGENTS.md`에는 규칙·경로만 추가하고 `/etc/codex/config.toml` developer instruction과 MCP description에도 bounded lookup/post-change 규칙 제공
...
- public 0.3.0 read-only audit의 catalog FAIL과 진단 손실은 자동 0.3.1 회귀로 수정하되 실제 HAOS 공개 image 재시험 전에는 live 완료로 승격하지 않는다.
