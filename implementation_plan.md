...
- `ha-memory-ha-client.mjs`의 Core WebSocket client와 root-only runtime env allowlist 전달
- entity/device/area registry list, `get_states`, `automation/config`, `search/related` command allowlist
- object/relationship allowlist normalization, complete snapshot staging과 atomic revision commit
...
- unsupported command, 일부 automation 실패, malformed/oversized field와 transport interruption
