...
App 재설치 전까지 `/data` 인증, 사용자 Codex 설정과 SSH host key가 유지된다. browser context와 output은 enforcement proxy가 `/run`의 일시 데이터로 제한하며 App 업데이트에 필요한 영속 상태로 취급하지 않는다. 설정 변경은 Git checkpoint 및 Home Assistant 설정 검사 절차를 권장한다.
