...
- 같은 image-managed system config의 `developer_instructions`와 MCP navigation tool 설명은 Home Assistant dashboard 요청에 `http://127.0.0.1:8099/`와 Playwright MCP를 첫 경로로 지정한다. 기존 사용자 config나 `AGENTS.md`를 갱신하기 위해 덮어쓰지 않는다.
