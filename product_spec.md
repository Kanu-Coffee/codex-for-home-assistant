...
App 재설치 전까지 `/data`의 Codex 인증, 사용자 Codex 설정, SSH host key와 관리형 browser identity recovery state가 유지된다. 사용자가 명시적으로 user-file refresh를 선택한 경우에는 기존 config/지침을 root-only backup으로 남기고 target별 version 적용 기록으로 재시작 반복을 막는다. browser context와 screenshot/output은 enforcement proxy가 `/run`의 일시 데이터로 제한하며 App 업데이트에 필요한 영속 상태로 취급하지 않는다. 설정 변경은 Git checkpoint 및 Home Assistant 설정 검사 절차를 권장한다.
