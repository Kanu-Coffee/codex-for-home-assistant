...
- App option의 browser token은 Supervisor가 영속 관리하고, init과 MCP launcher가 고정된 내부 `ws://supervisor/core/websocket`에서 user ID·non-admin·active·local-only·유일한 `system-read-only` group을 교차검증한 경우에만 mode 0600 `/run` 파일에서 browser init script 환경으로 전달. inherited browser token과 WebSocket endpoint는 무조건 제거하고 검증 파일에서만 token을 다시 주입
...
1. 전용 사용자의 long-lived token을 Home Assistant profile에서 즉시 revoke
2. App의 `home_assistant_browser_token` option을 지우고 재시작
...
5. 필요하면 새 token을 발급하고 exact read-only/local-only policy 검증 후에만 다시 설정
