...
- Core WebSocket 수집은 entity/device/area registry, `get_states`, `automation/config`, `search/related` allowlist로 제한하고 모든 필수 응답·정규화가 성공한 snapshot만 transaction commit. unavailable automation의 legal `config: null`은 빈 config와 bounded warning으로 수용하되 command/envelope/related 실패는 부분 성공으로 승격하지 않음
...
- registry/get_states/`automation/config`·`search/related` fixture에서 allowlist field와 relation만 저장되고 raw state/비허용 attributes/config/response/conversation/secret이 DB·FTS·audit·log에 없음을 검사. active unavailable automation의 `config: null`과 실제 installed `ws` handshake를 포함함
...
검증형 메모리는 fixture의 SQLite/FTS5, state machine, allowlist와 failure injection이 통과해도 실제 HAOS의 Core WebSocket command 가용성, registry/automation 규모, Core restart 재연결, App update persistence와 S6 lifecycle을 완료로 간주하지 않는다. Public 0.3.0의 read-only HAOS 감사에서는 첫 catalog bootstrap이 FAIL했고, restart/candidate/change/privacy와 published 0.3.1 재시험은 NOT RUN이다. 후속 실제 HAOS E2E 결과를 별도로 기록하기 전까지 이를 자동 회귀로 대체하지 않으며, 실제 entity·area·automation 이름이나 memory DB를 저장소 증거로 커밋하지 않는다.
